...
If you want to contribute to the project or test local changes, you can link your local development version:
...
#### 2. Link Local Version

Replace your globally installed `ccr` with the local development version:

```bash
npm link
```

This creates a symlink from your global `ccr` command to your local development version. Now any changes you make will be reflected when you run `ccr` commands (after rebuilding).
...
```bash
npm run build
ccr restart  # Restart the service to use the new build
ccr code     # Test your changes
```

#### 4. Unlink and Restore Global Version

When you're done with development and want to switch back to the stable globally installed version:
